<template>
    <div>
        ...
        <div class="flex items-center justify-between mb-5 sm:mb-8">
            <div>
                <h1 class="text-xl sm:text-3xl font-bold text-white">{{ t('settings.title') }}</h1>
                <p class="text-xs sm:text-sm text-white/40 mt-0.5 sm:mt-1">{{ t('settings.subtitle') }}</p>
            </div>
        </div>
        ...
        <div class="glass-card p-4 sm:p-6 mb-6 sm:mb-8">
            ...
            <form v-else @submit.prevent="saveSettings">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
                    <!-- Sync Interval -->
                    <div>
                        <label for="settings-sync-interval" class="block text-xs font-medium text-white/40 mb-1.5 sm:mb-2 uppercase tracking-wider">{{ t('settings.sync_interval') }}</label>
                        <div class="relative">
                            <select id="settings-sync-interval" v-model.number="form.sync_interval" :aria-label="t('settings.sync_interval')" class="glass-select w-full text-xs sm:text-sm">
                    ...
                    <!-- Log Retention -->
                    <div>
                        <label for="settings-log-retention" class="block text-xs font-medium text-white/40 mb-1.5 sm:mb-2 uppercase tracking-wider">{{ t('settings.log_retention') }}</label>
                        <div class="relative">
                            <select id="settings-log-retention" v-model.number="form.log_retention_days" :aria-label="t('settings.log_retention')" class="glass-select w-full text-xs sm:text-sm">
        ...
        <!-- Danger Zone -->
        <div class="glass-card p-4 sm:p-6 border-red-500/20 bg-red-500/[0.02]">
            <div class="flex items-center gap-3 mb-4 sm:mb-5 border-b border-red-500/10 pb-3">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                <!-- Clear Logs -->
                <div class="p-3.5 sm:p-4 rounded-xl border border-white/5 bg-white/[0.01] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
                    <div class="min-w-0">
                        <h3 class="text-sm font-semibold text-white/90">{{ t('settings.clear_logs') }}</h3>
                        <p class="text-xs text-white/30 mt-0.5 sm:mt-1">{{ t('settings.clear_logs_hint') }}</p>
                    </div>
                    <button @click="clearLogs" :disabled="clearingLogs"
                            class="btn-danger w-full sm:w-auto flex items-center justify-center gap-1.5 py-2.5 px-4 text-xs font-semibold disabled:opacity-60 shrink-0">
                ...
                <div class="p-3.5 sm:p-4 rounded-xl border border-white/5 bg-white/[0.01] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
                    <div class="min-w-0">
                        ...
                        <p class="text-xs text-white/30 mt-0.5 sm:mt-1">{{ t('settings.pause_tasks_hint') }}</p>
                    </div>
