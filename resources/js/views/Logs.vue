<template>
    <div>
        <!-- Page Header -->
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 mb-5 sm:mb-8">
            <div>
                <h1 class="text-xl sm:text-3xl font-bold text-white">{{ t('logs.title') }}</h1>
                <p class="text-xs sm:text-sm text-white/40 mt-0.5 sm:mt-1">{{ t('logs.subtitle') }}</p>
            </div>
            <button @click="loadLogs(1)" :disabled="loading" class="btn-secondary w-full sm:w-auto flex items-center justify-center gap-2 py-2 px-4 text-xs sm:text-sm shrink-0">
        ...
        <!-- Filter Bar -->
        <div class="glass-card p-3.5 sm:p-4 mb-4 sm:mb-6 grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <!-- Account Selector -->
            <div>
                <label for="logs-filter-account" class="block text-xs font-medium text-white/40 mb-1.5 sm:mb-2 uppercase tracking-wider">{{ t('logs.filter_account') }}</label>
                <div class="relative">
                    <select id="logs-filter-account" v-model="filter.accountId" @change="onFilterChange" :aria-label="t('logs.filter_account')" class="glass-select w-full text-xs sm:text-sm">
            ...
            <!-- Log Level -->
            <div>
                <label for="logs-filter-severity" class="block text-xs font-medium text-white/40 mb-1.5 sm:mb-2 uppercase tracking-wider">{{ t('logs.filter_severity') }}</label>
                <div class="relative">
                    <select id="logs-filter-severity" v-model="filter.level" @change="onFilterChange" :aria-label="t('logs.filter_severity')" class="glass-select w-full text-xs sm:text-sm">
        ...
        <div v-if="pagination.last_page > 1" class="flex flex-col sm:flex-row items-center justify-between gap-2.5 sm:gap-3 mb-4">
        ...
        <!-- Logs Listing -->
        <div class="glass-card overflow-hidden mb-4 sm:mb-6 relative w-full">
            <!-- Skeleton placeholders on first load -->
            <div v-if="loading && logs.length === 0" class="divide-y divide-white/5">
                <div v-for="i in 8" :key="'log-skeleton-' + i" class="p-3 sm:p-4 flex items-center gap-3 sm:gap-4">
                    <div class="w-14 sm:w-16 h-5 rounded-full skeleton flex-shrink-0"></div>
                    <div class="flex-1 min-w-0">
                        <div class="w-2/3 h-3 rounded skeleton mb-2"></div>
                        <div class="w-1/3 h-2 rounded skeleton"></div>
                    </div>
                    <div class="w-16 sm:w-20 h-3 rounded skeleton"></div>
                </div>
            </div>
            <div v-else-if="logs.length > 0" class="divide-y divide-white/5">
                <log-entry v-for="log in logs" :key="log.id" :log="log" />
            </div>
            <div v-else class="p-8 sm:p-12 text-center text-white/30 text-xs sm:text-sm">
                <p>{{ t('logs.empty') }}</p>
            </div>
        ...
        <div v-if="pagination.last_page > 1" class="flex flex-col sm:flex-row items-center justify-between gap-2.5 sm:gap-3">
            <p class="text-xs text-white/30">
                {{ t('logs.page_of', { current: pagination.current_page, last: pagination.last_page }) }}
            </p>
            <div class="flex items-center gap-2 w-full sm:w-auto">
