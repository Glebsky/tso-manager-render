        ...
        <div class="glass-card max-w-2xl w-full flex flex-col max-h-[90vh] sm:max-h-[85vh] shadow-2xl border border-white/10">
            <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-white/5 flex items-center justify-between flex-wrap gap-2">
                <div class="flex items-center gap-2 sm:gap-3">
                    <h3 class="text-sm sm:text-base font-semibold text-white">{{ t('tasks.modal.select_specialist') }}</h3>
                    <span class="badge badge-emerald text-[10px] sm:text-xs font-mono">{{ t('tasks.modal.selected_count', { count: selectedSpecialists.length }) }}</span>
                </div>
            ...
            <div class="p-3 sm:p-4 border-b border-white/5 bg-white/[0.01]">
                <input :value="specialistSearch" @input="$emit('update:specialistSearch', $event.target.value)" type="text" :placeholder="t('tasks.modal.search_specialist')" :aria-label="t('tasks.modal.search_specialist')" class="glass-input w-full text-xs py-1.5 sm:py-2 pl-3 sm:pl-4">
            </div>
            <div class="p-3 sm:p-6 overflow-y-auto flex-1 bg-dark-950/20">
                <div v-if="filteredSpecialistsModal.length > 0" class="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">
