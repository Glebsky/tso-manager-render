    ...
    <div v-if="showModal" @click.self="$emit('close')" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
        <div class="glass-card max-w-2xl w-full flex flex-col max-h-[80vh] shadow-2xl border border-white/10">
            <div class="px-6 py-4 border-b border-white/5 flex items-center justify-between">
                ...
                <button type="button" @click="$emit('close')" aria-label="Close" class="text-white/40 hover:text-white transition-colors">
            ...
            <div class="p-4 border-b border-white/5 bg-white/[0.01]">
                <input :value="buffSearch" @input="$emit('update:buffSearch', $event.target.value)" type="text" :placeholder="t('tasks.modal.search_buff')" :aria-label="t('tasks.modal.search_buff')" class="glass-input w-full text-xs py-2 pl-4">
            </div>
