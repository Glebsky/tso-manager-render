...
**Ловушка.** Значения 0–11 — это одновременно и «семантический вид про��укции», и
...
## 7. Пробел в снапшоте зоны (ЗАКРЫТ)

> **Статус на версию 2.0.** Пробел в ЗНАНИЯХ закрыт: есть живой снапшот
> зоны с непустой очередью производства. Фактические имена полей, пути и
> ловушки — в `zone-snapshot-evidence.md`, целевая схема ключей — в `design.md` §4.
> Главные факты: очереди лежат в `dZoneVO.timedProductions_vector`; поля
> `productionType` у здания в снапшоте НЕТ (берётся из каталога);
> индекс коллекции ≠ `productionType` (P-26); `buildingGrid` у заказа = 0 (P-24).
> Ниже сохранён анализ ТЕКУЩЕГО парсера — он всё ещё не умеет ничего
> из этого, и это по-прежнему обязательный объём работ (этап 2).
...
| Нужно | Зачем |
| --- | --- |
| `production_type` здания (из каталога по `buildingName_string`) | ключ очереди; отбор зданий-производителей |
| `upgradeLevel` + `upgradeIsInProgress` | фильтр `requiresUpgradeLevelMin/Max`; запрет во время апгрейда |
| `dZoneVO.timedProductions_vector` | сколько заказов уже стоит (искать по `orders[0].productionType`, НЕ по индексу) |
