...
> **Куда положить:** содержимое этой папки → `docs/references/amf/`
> в репозитории `admin`. Это постоянный референс для **любых**
...
| `amf/callNN_request.body.bin` | тело запроса, восстановленное из JS-строки (С ПОТЕРЯМИ, см. ниже) |
| `amf/callNN_request.strings.txt` | ASCII-строки из запроса — классы, traits, handler, operation |
| `amf/callNN_response.bin` | ответ сервера, байт-в-байт (годится как тестовая фикстура) |
| `amf/callNN_response.b64` | то же в base64 |
| `amf/callNN_response.decoded.json` | декодированный ответ |
| `amf/call03_response.decoded.json.gz` | снапшот зоны в JSON (сжат, развёрнутый ~10 МБ) |
| `amf/call03_response.schema.json` | структурная схема `dZoneVO`: пути, имена классов, типы, длины коллекций |
...
```bash
python3 decode.py amf/call03_response.bin | less
...
```python
import amf3
pkt = amf3.decode_packet(open('amf/call03_response.bin', 'rb').read())
