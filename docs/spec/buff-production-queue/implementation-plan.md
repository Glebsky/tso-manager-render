...
## Этап 0. Закрыть блокеры — ВЫПОЛНЕН (3/3)

> **Статус на версию 2.0: этап закрыт, кодить можно с этапа 1.**
> 1. OQ-1 закрыт — 4 ветви `SetFilteredReciepeList`, см. `oq-resolutions.md`.
> 2. Фикстура зоны закрыта — `references/amf/call03_response.bin` (595 147 байт,
>    живой заказ `Tome`, `productionType = 2`) → копировать в
>    `tests/Fixtures/Amf/zone_snapshot_1002.bin`.
> 3. OQ-3 закрыт — оператор подтвердил PHP **8.5**; `constitution.md`
>    авторитетен, `laravel-constitution.md` устарел (F-8).
>
> Дополнительно закрыто сверх плана: формат VO команды 91 (14 полей,
> ADR-3-R), путь очередей в снапшоте, общность очереди по `productionType`.
> Остаются открытыми OQ-4 и OQ-5 — **ни один не блокирует реализацию**
> (обходы описаны в `oq-resolutions.md`).
> **Этап 2 разблокирован.**

Историческая формулировка этапа (все три пункта выполнены):
...
- С��ежий снапшот: `Account::$zone_data` ни разу не читается (AC-9).
...
| Кл��чи очередей в снапшоте зоны | блокер | ЗАКРЫТО (`timedProductions_vector`) |
